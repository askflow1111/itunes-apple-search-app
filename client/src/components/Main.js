...
import { updateItemInfoModal, areTwoArrSame } from ".././actions/itunesAppActions";
...
class Main extends Component {
  ...
  shouldComponentUpdate(newProps, nextState) {
    if (!this.props.dispatch(areTwoArrSame(this.props.itunesAppStore.itunesApiData, newProps.itunesAppStore.itunesApiData)) || (this.props.itunesAppStore.fetched !== newProps.itunesAppStore.fetched) || (this.props.match.path !== newProps.match.path)) {
      return true;
    } else {
      return false;
    }
  ...
  renderResults = () => {
    if (this.props.itunesAppStore.fetched) {
      return <Results />
    } else {
  ...
  renderResultsHeader = () => {
    return (
      <div className="results-header">
        <div className="results-count">Showing {(this.props.itunesAppStore.itunesApiData === "") ? 0 : this.props.itunesAppStore.itunesApiData.length} results</div>
        <SortBy />
      </div>
    )
