export default function reducer(
  state = {
    ...
    shouldFetch: true,
    fetched: false,
    userInput: { listingType: "all", searchBoxValue: "", sortBy: "Best Match", maxResults: 25 },
    error: null,
  ...
  switch (action.type) {
    case "FETCH_ITUNESAPI_REJECTED": {
      return { ...state, fetched: true, error: action.payload, itunesApiData: [] };
    }

    case "FETCH_ITUNESAPI_FULFILLED": {
      return {
        ...state,
        shouldFetch: false,
        fetched: true,
        itunesApiData: action.payload.itunesApiData,
        comingFromInput: false,
      };
    }
    ...
    case "ADD_FAV_ITEM_TO_DB_FULFILLED": {
      return {
        ...state,
        favDataFromDB: { ...state.favDataFromDB, [action.payload.uniqueId]: action.payload },
      };
    }
    ...
    case "CHANGE_USERINPUT_ListingType": {
      return {
        ...state,
